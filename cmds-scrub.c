...
static void print_scrub_summary(struct btrfs_scrub_progress *p, struct scrub_stats *s, u64 bytes_total)
{
	...
	u64 seconds_left   = (bytes_per_sec > 0) ? (bytes_total - bytes_scrubbed) / bytes_per_sec : 0;
	unsigned h_left = seconds_left / 3600;
	unsigned m_left = (seconds_left / 60) % 60;
   ...
   "failed to write the progress status file: %m. Status recording disabled");
	...
	if (do_print) {
		...
		for (i = 0; i < fi_args.num_devices; ++i) {
			...
			bytes_total += di_args[i].bytes_used;
		}
		if (!do_stats_per_dev) {
			printf("scrub %s for %s\n", append, fsid);
			print_fs_stat(&fs_stat, print_raw, bytes_total);
		}
	}
