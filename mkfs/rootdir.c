...
static s32 read_from_source(const struct btrfs_fs_info *fs_info, const char *path,
			    int fd, char *buf, u64 filepos, u32 length)
{
	u32 cur = 0;
			      ...
			      int type, struct FTW *ftwbuf)
{
	...
	if (S_ISREG(st->st_mode) && st->st_size > 0) {
		...
		if (st->st_nlink > 1) {
			...
			if (ret < 0) {
				errno = -ret;
				error("failed to add hard link record for '%s': %m", fpath);
				return ret;
			}
		}
